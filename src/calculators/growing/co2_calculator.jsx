            ...
            <div>
                <div className="row">
                    <div className="col-sm">
                        ...
                        <GenericInput inputLabel="Room Length" onChange={(val) => this.setState({growSpaceLengthFt: Number(val)})} conversionFactors={ConversionFactors.basicDistance} number={this.state.growSpaceLengthFt}/>
                        <GenericInput inputLabel="Room Height" onChange={(val) => this.setState({growSpaceHeightFt: Number(val)})} conversionFactors={ConversionFactors.basicDistance} number={this.state.growSpaceHeightFt}/>
                        <FixedUnitInput inputLabel="Ambient CO₂" onChange={(val) => this.setState({ambientPPM: Number(val)})} number={this.state.ambientPPM} unit="ppm"/>
                        <FixedUnitInput inputLabel="Desired CO₂" onChange={(val) => this.setState({desiredPPM: Number(val)})} number={this.state.desiredPPM} unit="ppm"/>
                        <FixedUnitInput inputLabel="Minutes On" onChange={(val) => this.setState({minutesOn: Number(val)})} number={this.state.minutesOn} unit="per hour"/>
                        <GenericInput inputLabel="Volume Compensation*" onChange={(val) => this.setState({volumeReductionFt3: Number(val)})} conversionFactors={ConversionFactors.basicVolume} number={this.state.volumeReductionFt3}/>
                    </div>
                    <div className="col-sm">
