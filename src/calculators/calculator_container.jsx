...
export default class CalculatorContainer extends React.Component {
    ...
    componentDidMount() {
        ...
        if (window.location.hash.length > 1) { // don't do this on bare URL lest you redirect loop
            setTimeout(() => {
    ...
    render() {
        return (<div id="mainAccordion">
            <input aria-label="search for calculators" value={this.state.searchTerm} onChange={this.setSearchTerm} placeholder="Enter a keyword or term to search for a calculator..." id="searchBar"/> {
                this.isSearching()
                    ? ""
            ...
            <CardTemplate id="flowRateConversion" keywords="flow rate speed volume cubic standard ^3 per minute second hour hr day centimeters cm minutes gallons gpm inches liters scfm scfh" searchTerm={this.state.searchTerm} title="Flow Rate Conversion" parentID="mainAccordion" hideSplitter="hideSplitter">
            ...
            <CardTemplate id="TDSDilution" keywords="tds dilution water ppm" searchTerm={this.state.searchTerm} title="Dilution to TDS Target" parentID="mainAccordion">
                <TDSDilution/>
            </CardTemplate>
